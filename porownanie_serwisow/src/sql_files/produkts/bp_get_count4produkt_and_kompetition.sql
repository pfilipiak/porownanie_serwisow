create or replace function bp_get_count4produkt_and_kompetition(
    p_opis text
)
returns setof tcout_setof as
$$
declare
    v_wynik tcout_setof;
    v_query text;
...
begin
    ...
    for v_row2 in select *
        ...
        for v_row3 in select *
            ...
            loop
             v_query := 'select '||E'\'' || v_row.recognize_text ||E'\''||' as nazwa, count(*) as count, month, year, '|| v_row2.id || ' as id_slownik_branded ,' ||v_row3.id|| ' as id_slownik_pozycja ' 
...
--select bp_get_count4produkt_and_kompetition('saturn.pl');
...
$$
language plpgsql;
select bp_get_count4produkt_and_kompetition('saturn.pl',1,1);